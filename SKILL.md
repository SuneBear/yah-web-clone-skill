...
name: yah-web-clone
description: >
  Yah Web Clone v3 网站复刻工作流，默认使用中文交付，支持三种明确模式：(1) full：严格 1:1 镜像并附中文解构、关键截图、可运行效果实验室、GUI 与 Preset；(2) mirror：只保存可本地运行的网站缓存，不做额外分析；(3) effect：不克隆整站，只忠实复刻单个 WebGL、Canvas、Three.js、Shader、滚动或交互动效并提供可调参数；并用结构化分类、README 与 GitHub Topics 支持后续检索。Use when 用户明确指定 Yah Web Clone、$yah-web-clone，或要求克隆网站、mirror site、离线缓存、分析网站特效、提取单个效果、创建可迁移实验代码、分类复刻项目、推送 GitHub 或部署 Cloudflare Pages。
---
...
默认模式始终是 `full`。只有用户明确说“只缓存”“不要分析”、显式传入 `--mode mirror`，或本机配置明确覆盖时才使用 `mirror`；只有用户明确只要单个效果、显式传入 `--mode effect`，或本机配置明确覆盖时才使用 `effect`。意图不明确时不要猜成更轻的模式。不要再把 `analysis`、`study` 当成交付模式；分析与可读重建是 `full` 或 `effect` 的内置阶段。

项目和仓库统一命名为 `<slug>-clone`。把用户输入末尾已有的 `-clone`、`-mirror` 或叠加后缀归一为单个 `-clone`；`mirror` 只作为模式名，不进入项目后缀。不要自动重命名已有仓库，除非用户明确要求迁移远端、部署与链接。
...
完成侦察后为每个项目填写内容分类，供 GitHub 与 Sune Library 检索。把 `clone.config.json.catalog` 作为唯一分类源；README 分类区和 GitHub Topics 由 `yah catalog` 投影，不要手工维护三份。只记录技术、能力、视觉、主题与中文/自然语言关键词；不要添加 clone、mode 或 workflow 系统标签，也不要猜测没有证据的分类。
...
```text
<project>/
├── site/                 # 原样镜像；effect 模式不创建
├── lab/                  # 独立可运行的效果；mirror 模式不创建
...
不要创建 `RECON/`、`study-src/` 或独立 Storybook。把给人看的结果放 `docs/`，把可运行重建放 `lab/`，把原镜像放 `site/`。
...
读取 `references/effect-extraction.md` 获取 RAW REPLAY、证据分级和 baseline 闸门；读取 `references/deliverables.md` 获取各模式验收清单；分类或同步 GitHub Topics 时读取 `references/catalog.md`。需要录制动效时读取 `references/motion-capture.md`。只有静态部署镜像时读取 `references/static-mirror.md`；只有发布 Cloudflare 时读取 `references/cloudflare-pages.md`。

## 来源与忠实度

按以下优先级使用证据：固定 commit 的公开源码 > source map > 部署 bundle > 运行时捕获 > 视觉推断。将每个关键结论标为 `SOURCE`、`PARTIAL` 或 `GUESS`；AI 重建默认是 `GUESS`。

严格 1:1 至少覆盖页面结构、文案、字体、颜色、间距、资产、桌面/平板/手机、深层路由、关键交互状态、Canvas/WebGL 真渲染和 console/page error。单张桌面首屏或“0 error”不能证明 1:1。无法完成时停止声称 1:1，说明缺少什么以及证据在哪里。
...
- `prepare-deploy.mjs`：按模式组合静态发布目录，并在 full 模式挂载构建后或静态 Lab。
