...
## 一键脚本
```bash
node ~/.codex/skills/yah-web-clone/scripts/mirror-site.mjs \
  --url https://<站>/ \
  --out "$YAH_PROJECT"
```
产物：
- `<out>/site/…`：镜像的**同源**资产（保留路径；目录 URL 存 `index.html`）
- `<out>/.clone/evidence/mirror/own-assets.txt`：同源资产清单
- `<out>/.clone/evidence/mirror/third-party.json`：第三方 host + **需自托管的 webfont CSS**（Typekit/Google）提示
- `<out>/.clone/evidence/mirror/summary.json`：请求数量、成功/失败与来源摘要
- `<out>/.clone/work/mirror/request-manifest.json`：完整请求过程清单，验收后可清理

脚本用真浏览器全程滚动捕获 + 走浏览器网络栈下载（cookie/TUN/代理与页面一致）。

## 镜像后的手工收尾（让它离线 1:1 跑）
脚本只搬同源资产、不自动改写——**第三方按 `.clone/evidence/mirror/third-party.json` 人工处理**：
...
```
然后按 SKILL.md 的“忠实度闸门”执行路由 × 视口 × 状态截图矩阵，并检查浏览器 console/page error。重 WebGL 站记得**滚动到各段截图**对照（静态全页截图抓不到滚动触发的 GL 帧）。
...
- 完整记录：`<workspace-root>/oryzo-clone/`（NOTES.md + TEARDOWN.md）
