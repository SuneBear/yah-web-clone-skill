...
> 它和 yah-web-clone 头号铁律「真源码至上」同魂，只是把"标行号"升级成了系统化的证据制度。
...
- 对应 yah-web-clone 老规矩：验证不了的如实写，**别伪造"拖动成功"**。
...
yah-web-clone 默认先用 `mirror-site.mjs` 获取部署资产和真实请求，再去 GitHub / source-map 补真源码。但**特效站经常无源、minified 到底**。
...
**遇到下面情况直接把特效那一段委托给它，yah-web-clone 只当总入口**：

- 站是 WebGL/WebGPU/重 Canvas 特效，且 GitHub + source-map 都找不到真源码；
- 需要运行时帧捕获、逐帧比对、shader/uniform 级别的抠取；
- 想要"先 baseline 复现、再独立工程化"的完整带闸流程。

委托产物拿回来后，把原始 baseline 和证据包放入 `$YAH_PROJECT/.clone/work/baseline/`，
把通过闸门后的可读工程化实现放入 `$YAH_PROJECT/lab/effects/`。补充 `docs/ANALYSIS.md`，并执行
本地验证、文档、GitHub、Cloudflare 和生产验证阶段。
...
- baseline 原始捕获先放 `<站名>-clone/.clone/work/baseline/`；只把支撑结论的摘要提升到 `.clone/evidence/`。
