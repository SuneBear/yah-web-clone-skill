...
- `full` 的组合布局默认为 `site/ → /`、Lab 产物 `→ /__lab/`；`mirror` 只发布 `site/`；`effect` 把 Lab 产物发布到 `/`。
...
- 根路径和主要深层路由返回成功状态。
- `full` 还要验证 `/__lab/`、至少一个 `lab/effects/<effect>/` 深层入口、Lab 的 JS/CSS chunk 和 GUI / Preset；不能只确认 HTML 为 200。
