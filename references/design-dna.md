# Design DNA · 按需的结构化设计身份层

把“做成那个站的感觉”从模糊感觉变成**可版本化、可复用、可对照的 JSON 规范**。仅在用户明确要求提取设计系统、制作风格变体，或让 Collection 做跨案例设计综合时使用：侦察之后、实现变体之前，多产出一个 `design-dna.json`。它不是默认交付，也不是检索 Meta。

> 方法与 schema 改编自 [zanwei/design-dna](https://github.com/zanwei/design-dna)（MIT），按 yah-web-clone 的口径裁剪。

## ⚠️ 适用边界（必须先读）

Design DNA 的生成哲学是**"近似风格"**——它产出的是"风格一致的新站"，不是逐字节复制。
这和 yah-web-clone 的头号铁律「真源码至上 / 逐字节忠实」是**相反方向**。所以：

| Yah 场景 | 用不用 DNA |
|---|---|
| `mirror` | **不用**。可运行源码与证据才是真相，别让 DNA 把它稀释成“近似” |
| `full` / `effect` | **通常不用**。只有用户同时要求设计系统提取或风格变体时才生成 |
| `collection` | **可选**。研究问题明确要求跨案例设计语言或 token 综合时生成 |
| 独立的风格移植请求 | **用**。DNA 定义保留项、替换项和可调变量，但不改变原版 baseline |

一句话：**Catalog Meta 服务检索，DNA 服务有意改造；两者都不参与默认的 1:1 镜像判定。** 面向人的 `docs/DESIGN.md` 如有需要应由 DNA/Meta 派生，不再成为第二份手工 Source of Truth。
...
3. **Generate** — 解析 DNA → 生成 CSS 自定义属性 → 按 `design_style` 做主观决策 → 按 `effect_intensity` 选实现层级（lightweight=CSS/SVG/vanilla；medium=Canvas2D/GSAP/Lottie；heavy=Three.js/GLSL/Pixi）→ 输出页面 → 灌入用户指定内容。素材选择遵循用户要求与适用边界。

## 脚手架

```bash
node ~/.codex/skills/yah-web-clone/scripts/dna-scaffold.mjs \
  --recon "$YAH_PROJECT/.clone/work/scans/original-scan.json" \
  --out   "$YAH_PROJECT/.clone/work/design-dna.json"
```
