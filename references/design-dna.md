# Design DNA · `adapt` 模式的结构化设计身份层

把"做成那个站的感觉"从模糊感觉变成**可版本化、可复用、可对照的 JSON 规范**。
仅在用户明确选择 `adapt` 模式时使用：侦察之后、搭工程之前，多产出一个
`design-dna.json`，让品牌、内容或视觉变体有据可依。
...
| Yah 模式 | 用不用 DNA |
|---|---|
| `mirror`（拿到源码、部署产物或运行时证据后忠实镜像） | **不用**。可运行源码与证据才是真相，别让 DNA 把它稀释成"近似" |
| `analysis`（只做侦察、证据和实现分析） | **通常不用**。除非分析目标明确包含设计系统提取 |
| `study`（整理干净、可读、可移植的研究代码） | **可选**。只在需要解释设计 token 时生成 |
| `adapt`（保留设计语法并改品牌、内容或效果参数） | **用**。DNA 定义保留项、替换项和可调变量 |

一句话：**DNA 服务于有意改造，不参与默认的 1:1 镜像判定。**
